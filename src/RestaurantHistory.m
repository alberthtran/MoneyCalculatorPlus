...
@implementation RestaurantHistory

@synthesize purchased_date,restaurant_record,email_msg,shareAgain;
...
- (void) viewWillAppear:(BOOL)animated {
    ...
    NSDate *myDate = [restaurant_record objectForKey:PURCHASED_DATE_KEY];
    NSDateFormatter *df = [NSDateFormatter new];
    [df setDateFormat:DISPLAY_DATE_FORMAT];
    
    self.title = [restaurant_record objectForKey:RESTAURANT_NAME_KEY];
    purchased_date.text = [df stringFromDate:myDate];
	email_msg.text = [restaurant_record objectForKey:EMAIL_MSG_KEY];   
...
- (void) emailNow: (id) sender{
    ...
    //NSString* emailBody = [[NSString alloc] initWithString:email_msg.text];
...
- (void)pushFacebookPost{
    ...
    postVC.msgToPost = [NSString stringWithFormat:@"%@",email_msg.text];
...
- (void)pushTwitterPost{
    ...
    postTwitterVC.msgToPost = [NSString stringWithFormat:@"%@",email_msg.text];
